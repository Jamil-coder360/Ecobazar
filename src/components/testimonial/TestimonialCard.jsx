import quort from "../../assets/quort.png";
import test_1 from "../../assets/test_1.png";
export default function TestimonialCard() {
  return (
    <div className="w-[424px] bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      {/* Quote Icon */}
      <div className="text-5xl font-serif text-green-200 leading-none mb-4">
        <img src={quort} alt="" />
      </div>

      {/* Review Text */}
      <p className="text-gray_700 text-base leading-[150%] font-normal mb-8">
        Pellentesque eu nibh eget mauris congue mattis mattis nec tellus.
        Phasellus imperdiet elit eu magna dictum, bibendum cursus velit sodales.
        Donec sed neque eget
      </p>

      {/* Footer: Avatar + Name + Stars */}
      <div className="flex items-center justify-between">
        {/* Avatar & Name */}
        <div className="flex items-center gap-3">
          <img
            src={test_1}
            alt="Eleanor Pena"
            className="w-12 h-12 rounded-full object-cover"
          />
          <div>
            <p className="font-bold text-gray-900 text-sm">Eleanor Pena</p>
            <p className="text-gray-400 text-sm">Customer</p>
          </div>
        </div>

        {/* Stars */}
        <div className="flex items-center gap-1">
          {[...Array(5)].map((_, i) => (
            <svg
              key={i}
              className="w-5 h-5 text-amber-500"
              fill="currentColor"
              viewBox="0 0 20 20"
            >
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.448a1 1 0 00-.364 1.118l1.287 3.957c.3.921-.755 1.688-1.54 1.118l-3.37-2.448a1 1 0 00-1.175 0l-3.37 2.448c-.784.57-1.838-.197-1.539-1.118l1.287-3.957a1 1 0 00-.364-1.118L2.063 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69l1.286-3.957z" />
            </svg>
          ))}
        </div>
      </div>
    </div>
  );
}
